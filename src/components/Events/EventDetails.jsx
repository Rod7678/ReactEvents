...
export default function EventDetails() {
  ...
  if(isError){
    content = (
      <div id="event-details-content" className='center'>
        <ErrorBlock title='Failed to Fetch Event' message={error.info?.message || 'failed to fetch event. please try again'}/>
      </div>
    )
  };
    ...
    <>
      {isDeleting && <Modal onClose={handleStopDelete}>
        ...
        <div className="form-actions">
          ...
          {isDeleteError && <ErrorBlock title="Failed to delete event" message={deleteError.info?.message || 'failed to delete events'}/>}
        </div>
      </Modal>}
      <Outlet />
      <Header>
